...
export default function ProfilePage() {
  ...
  const viewingOther = Boolean(requestedUserId && requestedUserId !== (me as any)?._id);
      ...
      <div className="max-w-2xl space-y-6">
        ...
        <section className="space-y-2">
          <div className="inline-flex items-center gap-2 rounded-full border border-primary-500/20 bg-primary-500/10 px-3 py-1 text-[11px] font-medium text-primary-200">
            ...
            <span>{viewingOther ? "Traveler profile (view only)" : "Your public traveler profile"}</span>
          </div>
          <div>
            <h1 className="text-2xl md:text-3xl font-semibold text-slate-50">
              {viewingOther ? `${profile.fullName || profile.name || "User"}` : "My profile"}
            </h1>
        ...
        <section className="relative overflow-hidden bg-slate-950/80 border border-slate-800 rounded-2xl p-5 md:p-6 space-y-3 text-sm shadow-xl shadow-black/40">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            <p>
              ...
              <br />
              <span className="text-slate-100">{profile?.fullName ?? profile?.name ?? "—"}</span>
            </p>
            <p>
        ...
        <section className="relative overflow-hidden bg-slate-950/80 border border-slate-800 rounded-2xl p-5 md:p-6 space-y-4 text-sm shadow-xl shadow-black/40">
          ...
          <div className="flex items-center justify-between gap-2">
            <div>
              <h2 className="text-lg font-semibold text-slate-50">About &amp; travel details</h2>
              <p className="text-xs text-slate-400 mt-0.5">{viewingOther ? "Public details" : "These details help others understand your style and experience."}</p>
            </div>
          </div>
          ...
          {viewingOther ? (
            ...
            <div className="space-y-3 text-sm text-slate-300">
              ...
              <div>
                ...
                <p className="mt-1 text-slate-400">{profile?.createdAt ? new Date(profile.createdAt).toLocaleDateString() : "-"}</p>
              </div>
            </div>
          ) : (
            // Editable UI for own profile (kept mostly as you had it)
            <div className="space-y-3">
              ...
              {message && (
                <p
                  className={`text-xs mt-2 px-3 py-2 rounded-lg border ${
                    isError ? "text-red-300 bg-red-500/5 border-red-500/40" : "text-emerald-300 bg-emerald-500/5 border-emerald-500/40"
                  }`}
                >
                  {message}
                </p>
              ...
              <div className="flex items-center justify-between gap-3 pt-1">
                <p className="text-[11px] text-slate-500">Your profile may be visible to other users on trip requests and matches.</p>
                <button
                  onClick={handleSave}
                  disabled={saving}
